 ...
 */
public class AsciiTableRenderer implements V2_TableRenderer {
    /**
    ...
    @Override
    public AsciiTableRenderer setWidth(V2_Width width) {
        if (width != null) {
            this.width = width;
        }
        return this;
    ...
    public RenderedTable render(V2_AsciiTable table, boolean singleLine) {
        this.rows.clear();
        //nothing to do
        ...
        for (V2_Row row : table.getTable()) {
            ...
            if (row instanceof ContentRow) {
                ...
                String[][] procColumns = RenderUtilities.createContentArray(crow.getColumns(), cols, crow.getPadding(), singleLine);
...
//		}
    ...
    @Override
    public AsciiTableRenderer setTheme(V2_TableTheme theme) {
        if (theme != null) {
